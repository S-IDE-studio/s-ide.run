...
s-ide.run の current release-driven development で使用する Operating Model を定義します。
...
- S-IDE product/source facts remain owned by their canonical S-IDE repositories rather than being duplicated into the website
...
- Astro/package scripts and the repository lockfile remain the website application toolchain authority. Bun is auxiliary for Agent Skills only.
- `scripts/fetch-docs.ts` output is derived content; record/fix its upstream identity rather than treating stale fetched docs as current S-IDE evidence.
- source validation, fetched-doc validation, build output, Vercel deployment, and public-site smoke evidence are separate stages bound to the candidate.

## Agent Skills lifecycle

- current `rebuildup/project-init` を `bunx skills` で project-local に追跡する。
...
- `skills-lock.json` は durable source/content metadata。
- `--global` は freshness evidence にしない。
