# s-ide.run Constitution

この文書は、s-ide.run の開発組織における最上位 contract です。
...
s-ide.run の開発組織の目的は current procedure への服従ではありません。
...
s-ide.run is the S-IDE public website. Its build pulls documentation before Astro validation/build, so fetched documentation identity and website source identity must remain explicit.

## Upstream source

`rebuildup/project-init@release-0-3-0` is the governance reconciliation baseline. Agent Skills are not frozen to that revision.
